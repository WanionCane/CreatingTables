...
public class CreatingGhostHandler<B extends GuiCreatingTable> implements IGhostIngredientHandler<B>
{
	@Override
	@Nonnull
	public <I> List<Target<I>> getTargets(@Nonnull final B gui, @Nonnull final I ingredient, final boolean doStart)
	{
		...
		if (ingredient instanceof ItemStack) {
			for (int i = 0; i < gui.inventorySlots.inventorySlots.size() - 37; i++) {
				...
				targets.add(new CreatingTarget<>(slot.slotNumber, new Rectangle(gui.getGuiLeft() + slot.xPos - 1, gui.getGuiTop() + slot.yPos - 1, 18, 18), gui));
			}
		}
		final Slot outputSlot = gui.inventorySlots.getSlot(gui.inventorySlots.inventorySlots.size() - 37);
		targets.add(new CreatingTarget<>(outputSlot.slotNumber, new Rectangle(gui.getGuiLeft() + outputSlot.xPos, gui.getGuiTop() + outputSlot.yPos, 16, 16), gui));
		return targets;
	}
